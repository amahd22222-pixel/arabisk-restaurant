  ...
  const renderGroup=(key,rows)=>{
    ...
    node.innerHTML=filtered.map(row=>{
      ...
      const controls=(row.status==='draft'
        ? (task.key==='blocked'
          ? '<button class="small-action task-board-unblock" data-task-action="unblock" data-id="'+revEsc(row.id)+'">حل العائق</button><button class="small-action task-board-history" data-task-action="history" data-id="'+revEsc(row.id)+'">سجل النشاط</button>'
          : task.key==='unassigned'
