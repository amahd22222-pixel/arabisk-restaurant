  ...
  try{
    ...
    const timeline=(profile.timeline||[]).map(item=>{
      ...
      const actionHtml=targetSection&&item.reference
        ...
        : item.type==='opportunity'&&customer.marketingOptIn&&item.reference
          ? '<button class="small-action customer360-timeline-jump" type="button" data-customer-opportunity="'+targetRef+'" data-opportunity-type="'+escapeHtml(item.opportunityType||'')+'">إنشاء إجراء</button>'
          : '';
