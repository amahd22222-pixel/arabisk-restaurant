...
function renderStats() {
  ...
  $('#count-reservations').textContent = reservations.length;
}

function renderProducts() {
  const query = $('#search').value.trim().toLowerCase();
  const filtered = products.filter((p) => !query || `${p.nameAr} ${p.nameEn}`.toLowerCase().includes(query));
  $('#products-body').innerHTML = filtered.map((p) => `<tr><td><strong>${escapeHtml(p.nameAr)}</strong><small>${escapeHtml(p.nameEn)}</small></td><td>${escapeHtml(categoryName(p.categoryId))}</td><td class="price">AED ${Number(p.price).toFixed(0)}</td><td><span class="status ${p.available ? 'on' : 'off'}">${p.available ? 'متاح' : 'مخفي'}</span></td><td class="actions"><button data-edit="${p.id}">تعديل</button><button data-delete="${p.id}" class="danger">حذف</button></td></tr>`).join('') || '<tr><td colspan="5" class="empty">لا توجد أصناف</td></tr>';
}

function renderCategories() {
  $('#categories-body').innerHTML = categories.map((c, index) => `<tr><td>${index + 1}</td><td><strong>${escapeHtml(c.nameAr)}</strong></td><td>${escapeHtml(c.nameEn)}</td><td><span class="status ${c.active ? 'on' : 'off'}">${c.active ? 'نشط' : 'مخفي'}</span></td></tr>`).join('') || '<tr><td colspan="4" class="empty">لا توجد أقسام</td></tr>';
}

function renderReservations() {
  const rows = reservations.map((r) => `<tr><td><strong>${escapeHtml(r.name)}</strong><small>${escapeHtml(r.notes || '—')}</small></td><td>${escapeHtml(r.date)}<small>${escapeHtml(r.time)}</small></td><td>${Number(r.guests)}</td><td dir="ltr">${escapeHtml(r.phone)}</td><td><span class="status ${r.status === 'confirmed' ? 'on' : r.status === 'cancelled' ? 'off' : 'pending'}">${statusLabel[r.status] || r.status}</span></td><td class="actions"><button data-status="confirmed" data-id="${r.id}">تأكيد</button><button data-status="cancelled" data-id="${r.id}" class="danger">إلغاء</button></td></tr>`).join('');
  $('#reservations-body').innerHTML = rows || '<tr><td colspan="6" class="empty">لا توجد حجوزات حاليًا.</td></tr>';
}
...
function showSection(sectionId) {
  ...
  const titleMap = { dashboard: 'إدارة ARABISK', products: 'إدارة الأصناف', categories: 'إدارة الأقسام', reservations: 'حجوزات الطاولات', orders: 'الطلبات', customers: 'العملاء', settings: 'إعدادات الموقع' };
...
async function load() {
  ...
  try {
    const results = await Promise.all([request('/api/categories'), request('/api/products'), request('/api/orders'), request('/api/customers'), request('/api/reservations')]);
    [categories, products, orders, customers, reservations] = results;
    renderStats(); renderProducts(); renderCategories(); renderReservations();
    ...
    $('#connection').textContent = 'متصل'; $('#connection').className = 'connected'; $('#error').textContent = '';
  } catch (error) {
    $('#connection').textContent = 'غير متصل'; $('#connection').className = 'disconnected'; $('#error').textContent = `${error.message}. تحقق من رابط الـAPI في الإعدادات.`;
  }
}
...
$('#refresh-reservations').addEventListener('click', load);
$('#modal').addEventListener('click', (event) => { if (event.target.id === 'modal') closeModal(); });

$('#products-body').addEventListener('click', async (event) => {
  const edit = event.target.dataset.edit; const del = event.target.dataset.delete;
...
$('#reservations-body').addEventListener('click', async (event) => {
  const id = event.target.dataset.id; const status = event.target.dataset.status;
  if (!id || !status) return;
  try { await request(`/api/reservations/${id}`, { method: 'PATCH', body: JSON.stringify({ status }) }); await load(); } catch (error) { alert(error.message); }
});
...
$('#product-form').addEventListener('submit', async (event) => {
  ...
  try { await request(editingId ? `/api/products/${editingId}` : '/api/products', { method: editingId ? 'PATCH' : 'POST', body: JSON.stringify(payload) }); closeModal(); await load(); } catch (error) { alert(error.message); }
});
