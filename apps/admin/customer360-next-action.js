...
function renderNextAction(profile,customerId){
  ...
  const html='<section class="customer360-next-action" data-customer360-next-action><div class="customer360-next-action-main"><span>الخطوة التالية للنظام</span><strong>'+nextActionEsc(decision.label||'مراجعة العميل')+'</strong><p>'+nextActionEsc(nextAction)+'</p><small>الحالة: '+nextActionEsc(nextActionState(decision.stateKey))+(decision.reason?' — '+nextActionEsc(decision.reason):'')+'</small></div><div class="customer360-next-action-side"><b>'+nextActionEsc(nextActionState(decision.stateKey))+'</b>'+value+button+'</div></section>';
