...
function setRouteMode(isCategoryPage){document.body.classList.toggle('category-route',isCategoryPage);homeSection.hidden=isCategoryPage;reservationSection.hidden=isCategoryPage;aboutSection.hidden=isCategoryPage;categoryView.hidden=isCategoryPage;categoryDetail.hidden=!isCategoryPage;if(!isCategoryPage){menuSection.scrollIntoView({behavior:'smooth',block:'start'})}else{window.scrollTo(0,0)}}
async function openCategoryPage(categoryId,{push=false}={}){const category=categories.find(c=>c.id===categoryId);if(!category)return;currentCategory=category;setRouteMode(true);searchInput.value='';grid.innerHTML='<div class="menu-loading"></div><div class="menu-loading"></div><div class="menu-loading"></div>';if(push){window.location.assign(`/menu/${categorySlug(category)}`);return}document.title=`${language==='en'?category.nameEn:category.nameAr} — ARABISK`;try{currentItems=await loadProducts(category.id);renderCategoryPage(category,currentItems)}catch(error){console.error(error);grid.innerHTML=`<p class="empty-menu">${language==='en'?'Unable to load this category.':'تعذر تحميل هذا القسم حاليًا.'}</p>`}window.scrollTo(0,0)}
function goHome(){window.location.assign('/')}
function initRoute(){const slug=categoryFromPath();if(slug){const found=categories.find(c=>categorySlug(c)===slug);if(found)openCategoryPage(found.id,{push:false});else goHome()}else setRouteMode(false)}
backButton.addEventListener('click',goHome);
searchInput.addEventListener('input',()=>renderCategoryItems(currentItems));
window.addEventListener('popstate',()=>initRoute());
categoryView.addEventListener('click',event=>{const card=event.target.closest('[data-category]');if(card){return;}});
document.querySelector('#lang-toggle').addEventListener('click',()=>{language=language==='ar'?'en':'ar';localStorage.setItem('ARABISK_LANG',language);applyLanguage();if(currentCategory)document.title=`${language==='en'?currentCategory.nameEn:currentCategory.nameAr} — ARABISK`});
