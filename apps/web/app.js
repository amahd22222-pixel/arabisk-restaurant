...
function render(items) {
  ...
  grid.innerHTML = filtered.map((item) => `<article class="item"><span class="cat">${escapeHtml(item.nameEn)}</span><div><h3>${escapeHtml(isEn ? item.nameEn : item.nameAr)}</h3><p>${escapeHtml(isEn ? (item.descriptionEn || 'A signature ARABISK selection.') : (item.descriptionAr || 'اختيار من قائمة ARABISK المميزة'))}</p></div><div class="price">AED ${Number(item.price).toFixed(0)}</div></article>`).join('') || `<p class="empty-menu">${isEn ? 'No matching items.' : 'لا توجد أصناف مطابقة.'}</p>`;
}
