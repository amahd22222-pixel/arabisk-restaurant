...
export function registerMediaRoutes(app, { products, storageReady, presign, persistState }) {
  app.post('/api/images/presign', (req, res) => {
    ...
    const key = `products/${productId}/images/${crypto.randomUUID?.() || `${Date.now()}-${Math.random().toString(36).slice(2)}`}-${fileName}`;
