export function registerProductRoutes(app, {
  ...
  app.post('/api/products', requireAdminApiKey, (req, res) => {
    const {
      categoryId, nameAr, nameEn, descriptionAr = '', descriptionEn = '',
      imageUrl = '', imageKey = '', price, available = true, videoKey = '',
      tags = [], dietary = [], spiceLevel = 0, chefChoice = false, isNew = false
    } = req.body || {};
    ...
    const product = {
      ...
      imageKey: cleanKey(imageKey),
      price: Number(price),
      available: Boolean(available),
      videoKey: cleanKey(videoKey),
      tags: normalizeList(tags, SMART_TAGS),
      dietary: normalizeList(dietary, DIETARY_TAGS),
      spiceLevel: Math.max(0, Math.min(3, Number(spiceLevel) || 0)),
      chefChoice: Boolean(chefChoice),
      isNew: Boolean(isNew),
  ...
  app.patch('/api/products/:id', requireAdminApiKey, (req, res) => {
    const product = products.find(item => item.id === req.params.id);
    if (!product) return res.status(404).json({ message: 'Product not found' });
    ...
    if (body.tags !== undefined) product.tags = normalizeList(body.tags, SMART_TAGS);
    if (body.dietary !== undefined) product.dietary = normalizeList(body.dietary, DIETARY_TAGS);
  ...
  app.delete('/api/products/:id', requireAdminApiKey, (req, res) => {
  ...
  app.post('/api/videos/presign', requireAdminApiKey, (req, res) => {
    ...
    if (!fileName || !['video/mp4', 'video/webm', 'video/quicktime'].includes(contentType)) {
      return res.status(400).json({ message: 'Only MP4, WebM and MOV videos are supported.' });
    }
    if (!Number.isFinite(size) || size < 1 || size > 120 * 1024 * 1024) {
      return res.status(400).json({ message: 'Maximum video size is 120 MB.' });
    }
  ...
  app.post('/api/videos/delete-presign', requireAdminApiKey, (req, res) => {
    if (!storageReady) return res.status(503).json({ message: 'Video storage is not configured on the web service.' });
    const productId = cleanText(req.body?.productId, 40);
