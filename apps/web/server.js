...
app.patch('/api/orders/:id',requireAdminApiKey,(req,res)=>{const order=orders.find(item=>item.id===req.params.id);if(!order)return res.status(404).json({message:'Order not found'});const now=new Date().toISOString();if(req.body?.status!==undefined){const nextStatus=String(req.body.status);if(!ORDER_STATUS_TRANSITIONS[order.status]?.has(nextStatus))return res.status(409).json({message:`Invalid order status transition: ${order.status} -> ${nextStatus}`});order.status=nextStatus;invalidateSmartSnapshot();if(nextStatus==='completed'){order.completedAt=order.completedAt||now;if(order.orderType==='pickup'&&order.phone){const customer=customers.find(item=>item.phone===order.phone);if(customer){customer.name=cleanText(order.name,80);customer.orderCount=Number(customer.orderCount||0)+1;customer.lastOrderAt=order.completedAt;}}revenue.recordEvent({eventName:'order_completed',sessionId:cleanText(order.sessionId,100),orderId:order.id,orderValue:order.total,metadata:{orderType:order.orderType}});}}if(req.body?.notes!==undefined)order.notes=cleanText(req.body.notes,300);order.updatedAt=now;persistState();return res.json(order);});
