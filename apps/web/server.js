...
const corsOptions = { origin(origin, callback){ if(!origin || allowedCorsOrigins.has(origin)) return callback(null,true); return callback(null,false); }, methods:['GET','POST','PATCH','DELETE','OPTIONS'], allowedHeaders:['Content-Type'] };
...
const reservationRate=new Map(); const RESERVATION_RATE_WINDOW_MS=10*60*1000; const RESERVATION_RATE_LIMIT=8;
const getClientKey=(req)=>String(req.ip||req.headers['x-forwarded-for']||'unknown').split(',')[0].trim().slice(0,120)||'unknown';
const reservationRateLimit=(req,res,next)=>{ const now=Date.now(); const key=getClientKey(req); const previous=reservationRate.get(key); if(!previous||now-previous.startedAt>=RESERVATION_RATE_WINDOW_MS){reservationRate.set(key,{startedAt:now,count:1});return next();} if(previous.count>=RESERVATION_RATE_LIMIT){const retryAfter=Math.max(1,Math.ceil((RESERVATION_RATE_WINDOW_MS-(now-previous.startedAt))/1000));res.setHeader('Retry-After',String(retryAfter));return res.status(429).json({message:'Too many reservation requests. Please try again later.'});} previous.count+=1; return next();};
setInterval(()=>{const cutoff=Date.now()-RESERVATION_RATE_WINDOW_MS*2; for(const [key,entry] of reservationRate) if(entry.startedAt<cutoff) reservationRate.delete(key);},RESERVATION_RATE_WINDOW_MS).unref();
...
const restoreStudio=registerStudioRoutes(app,{storageReady,presign,readJson,writeJson,deleteObject});
