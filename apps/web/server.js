...
const withMediaUrls=(product)=>({...product,imageUrl:product.imageKey&&storageReady?presign('GET',product.imageKey,900):(product.imageUrl||''),videoUrl:product.videoKey&&storageReady?presign('GET',product.videoKey,900):''});

const orders=[]; const customers=[]; const reservations=[];
const reservationRate=new Map(); const orderRate=new Map();
...
app.patch('/api/orders/:id',requireAdminApiKey,(req,res)=>{const order=orders.find(item=>item.id===req.params.id);if(!order)return res.status(404).json({message:'Order not found'});const allowed=['pending','confirmed','preparing','ready','completed','cancelled'];if(req.body?.status!==undefined&&!allowed.includes(req.body.status))return res.status(400).json({message:'Invalid order status'});if(req.body?.status!==undefined)order.status=req.body.status;if(req.body?.notes!==undefined)order.notes=cleanText(req.body.notes,300);order.updatedAt=new Date().toISOString();persistState();return res.json(order);});
...
app.post('/api/orders',orderRateLimit,(req,res)=>{const b=req.body||{};const name=cleanText(b.name,80),phone=cleanText(b.phone,40),orderType=['dine_in','pickup'].includes(b.orderType)?b.orderType:'dine_in',notes=cleanText(b.notes,300);const rawItems=Array.isArray(b.items)?b.items:[];if(!name||phone.length<5||phone.length>40||!rawItems.length||rawItems.length>30)return res.status(400).json({message:'name, phone and at least one order item are required'});const merged=new Map();for(const raw of rawItems){const productId=cleanText(raw?.productId,40),quantity=Number(raw?.quantity);if(!productId||!Number.isInteger(quantity)||quantity<1||quantity>20)return res.status(400).json({message:'Invalid order item.'});merged.set(productId,(merged.get(productId)||0)+quantity);}const items=[];let total=0;for(const [productId,quantity] of merged){const product=products.find(item=>item.id===productId&&item.available!==false);if(!product)return res.status(400).json({message:'One or more selected items are no longer available.'});const unitPrice=Number(product.price);if(!Number.isFinite(unitPrice)||unitPrice<0)return res.status(400).json({message:'Invalid product price.'});const lineTotal=unitPrice*quantity;items.push({productId:product.id,nameAr:cleanText(product.nameAr,160),nameEn:cleanText(product.nameEn,160),quantity,unitPrice,lineTotal});total+=lineTotal;}const now=new Date().toISOString();const order={id:nextOrderId(),name,phone,orderType,notes,items,total,status:'pending',createdAt:now,updatedAt:now};orders.push(order);const existing=customers.find(customer=>customer.phone===phone);if(existing){existing.name=name;existing.lastOrderAt=now;existing.orderCount=Number(existing.orderCount||0)+1}else customers.push({id:crypto.randomUUID(),name,phone,orderCount:1,lastOrderAt:now});persistState();return res.status(201).json({id:order.id,total:order.total,status:order.status});});
...
app.post('/api/products',requireAdminApiKey,(req,res)=>{const {categoryId,nameAr,nameEn,descriptionAr='',descriptionEn='',imageUrl='',imageKey='',price,available=true,videoKey=''}=req.body||{};if(!categoryId||!nameAr||!nameEn||!Number.isFinite(Number(price)))return res.status(400).json({message:'categoryId, nameAr, nameEn and numeric price are required'});if(!categories.some(category=>category.id===categoryId))return res.status(400).json({message:'Unknown category'});const product={id:nextProductId(),categoryId,nameAr:cleanText(nameAr),nameEn:cleanText(nameEn),descriptionAr:cleanText(descriptionAr),descriptionEn:cleanText(descriptionEn),imageUrl:cleanUrl(imageUrl),imageKey:cleanKey(imageKey),price:Number(price),available:Boolean(available),videoKey:cleanKey(videoKey),sortOrder:products.length+1};products.push(product);persistState();return res.status(201).json(withMediaUrls(product));});
app.patch('/api/products/:id',requireAdminApiKey,(req,res)=>{const product=products.find(item=>item.id===req.params.id);if(!product)return res.status(404).json({message:'Product not found'});const b=req.body||{};const oldImageKey=product.imageKey;const oldVideoKey=product.videoKey;if(b.categoryId!==undefined){if(!categories.some(category=>category.id===b.categoryId))return res.status(400).json({message:'Unknown category'});product.categoryId=b.categoryId;}if(b.nameAr!==undefined)product.nameAr=cleanText(b.nameAr);if(b.nameEn!==undefined)product.nameEn=cleanText(b.nameEn);if(b.descriptionAr!==undefined)product.descriptionAr=cleanText(b.descriptionAr);if(b.descriptionEn!==undefined)product.descriptionEn=cleanText(b.descriptionEn);if(b.imageUrl!==undefined)product.imageUrl=cleanUrl(b.imageUrl);if(b.imageKey!==undefined)product.imageKey=cleanKey(b.imageKey);if(b.price!==undefined){if(!Number.isFinite(Number(b.price)))return res.status(400).json({message:'Price must be numeric'});product.price=Number(b.price);}if(b.available!==undefined)product.available=Boolean(b.available);if(b.videoKey!==undefined)product.videoKey=cleanKey(b.videoKey);if(storageReady&&b.imageKey!==undefined&&oldImageKey&&oldImageKey!==product.imageKey)void deleteObject(oldImageKey);if(storageReady&&b.videoKey!==undefined&&oldVideoKey&&oldVideoKey!==product.videoKey)void deleteObject(oldVideoKey);persistState();return res.json(withMediaUrls(product));});
