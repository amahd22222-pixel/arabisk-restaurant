...
app.patch('/api/products/:id', (req, res) => {
  ...
  if (!product) return res.status(404).json({ message: 'Product not found' });
  const body = req.body || {};
  if (body.categoryId !== undefined) { if (!categories.some((c) => c.id === body.categoryId)) return res.status(400).json({ message: 'Unknown category' }); product.categoryId = body.categoryId; }
