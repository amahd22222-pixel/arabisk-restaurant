...
const productionCorsOrigins = new Set(['https://arabiskadmin-production.up.railway.app']);
const developmentCorsOrigins = new Set(['http://localhost:4174','http://127.0.0.1:4174','http://localhost:5173','http://127.0.0.1:5173']);
...
const allowedCorsOrigins = isProductionRuntime ? productionCorsOrigins : new Set([...productionCorsOrigins,...developmentCorsOrigins]);
...
app.patch('/api/orders/:id',requireAdminApiKey,(req,res)=>{const order=orders.find(item=>item.id===req.params.id);if(!order)return res.status(404).json({message:'Order not found'});if(req.body?.status!==undefined){const nextStatus=String(req.body.status);if(!ORDER_STATUS_TRANSITIONS[order.status]?.has(nextStatus))return res.status(409).json({message:`Invalid order status transition: ${order.status} -> ${nextStatus}`});order.status=nextStatus;invalidateSmartSnapshot();if(nextStatus==='completed')revenue.recordEvent({eventName:'order_completed',sessionId:cleanText(order.sessionId,100),orderId:order.id,orderValue:order.total,metadata:{orderType:order.orderType}});}if(req.body?.notes!==undefined)order.notes=cleanText(req.body.notes,300);order.updatedAt=new Date().toISOString();persistState();return res.json(order);});
