...
function setQty(id,next){const item=cart.find(row=>row.id===id);if(!item)return;if(next<=0)cart=cart.filter(row=>row.id!==id);else item.qty=Math.min(20,Math.max(1,Number(next)||1));saveCart(cart);render();}
...
async function submitOrder(event){
  ...
  try{
    ...
    saveLastOrder(data,{orderType,tableNumber,name,phone});saveCart([]);cart=[];document.querySelector('#cart-form').reset();syncCheckoutFields();render();status.textContent='';showSuccess(data);
  }catch(e){status.textContent=e.name==='AbortError'?'انتهت مهلة الاتصال.':(e.message||'تعذر إرسال الطلب.');}finally{submit.disabled=false;}
}
document.querySelector('#cart-clear').addEventListener('click',function(){if(!cart.length)return;if(window.confirm('هل تريد إفراغ السلة؟')){cart=[];saveCart(cart);render();}});
